...
def batch_norm(x, name, _ops, is_train=True):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        if is_train is True:
            ...
            variance = tf.compat.v1.get_variable('moving_variance', params_shape, tf.float32, trainable=False)
...
def instance_norm(x, name='instance_norm', mean=1.0, stddev=0.02, epsilon=1e-5):
    with tf.compat.v1.variable_scope(name):
        ...
        scale = tf.compat.v1.get_variable(
            ...
            initializer=tf.compat.v1.random_normal_initializer(mean=mean, stddev=stddev, dtype=tf.float32))
